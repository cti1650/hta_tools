Vue.component("SearchListBox", {
  ...
  template:
    '<div class="w-full">' +
    " <div" +
    '   class="fixed top-0 left-0 border-black border-l-8 border-b bg-gray-100 mb-2 py-1 pl-4 w-full h-auto"' +
    " >" +
    ...
    "   </div>" +
    " </div>" +
    ' <div id="contents" class="flex flex-col bg-gray-100 pt-16">' +
    '   <div v-for="item in items" :key="item.name">' +
    '     <div v-if="~item.name.toLowerCase().indexOf(keyword.toLowerCase())" class="mb-1">' +
    '       <a :href="item.path">' +
    '         <div class="text-xs text-gray-400">{{item.accessed ? item.accessed + " " + item.path : item.path}}</div><div class="pl-2 text-lg">{{item.name}}</div>' +
    "       </a>" +
    "     </div>" +
    "   </div>" +
    " </div>" +
